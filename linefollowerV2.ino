  ...
  <script>
    ...
    function majReglages() {
      ...
      envoiTimer = setTimeout(() => {
        fetch(`/set?base=${valeur('base')}&kp=${valeur('kp')}&kd=${valeur('kd')}&inv=${valeur('inv')}&mode=${valeur('mode')}&target=${valeur('target')}`);
      }, 120);
    }
...
void detecterMarqueurs()
{
  ...
  marqueurGaucheActif = centreSurLigne &&
                        (valeursCapteurs[0] > SEUIL_MARQUEUR ||
                         valeursCapteurs[1] > SEUIL_MARQUEUR);

  marqueurDroiteActif = centreSurLigne &&
