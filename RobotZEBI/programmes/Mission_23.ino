...
bool modePauseMarqueur = false;
bool pauseMarqueurActive = false;
...
uint16_t dureePauseMarqueurMs = 2000;
...
<body>
  <main>
    <header>
      <h1>LineFollower ESP32 - Mission 23</h1>
    ...
    <section class="grid">
      ...
      <article class="card wide">
        ...
        <label>
          <div class="row"><span>Pause a chaque marqueur</span><output id="pauseMarkOut">Non</output></div>
          <input id="pauseMark" type="range" min="0" max="1" step="1" value="0" oninput="majReglages()">
        </label>
        <label>
          <div class="row"><span>Duree pause marqueur</span><output id="pauseMarkMsOut">2000 ms</output></div>
          <input id="pauseMarkMs" type="range" min="500" max="5000" step="100" value="2000" oninput="majReglages()">
        </label>
        <label>
  ...
  <script>
    ...
    function majLabels() {
      ...
      pauseMarkOut.value = valeur('pauseMark') === '1' ? 'Oui' : 'Non';
      pauseMarkMsOut.value = `${valeur('pauseMarkMs')} ms`;
    ...
    function majReglages() {
      ...
      envoiTimer = setTimeout(() => {
        fetch(`/set?base=${valeur('base')}&kp=${valeur('kp')}&kd=${valeur('kd')}&mode=${valeur('mode')}&pauseMark=${valeur('pauseMark')}&pauseMarkMs=${valeur('pauseMarkMs')}&target=${valeur('target')}&us=${valeur('us')}&usDist=${valeur('usDist')}&inter=${valeur('inter')}&seq=${encodeURIComponent(valeur('seq'))}&adv=${valeur('adv')}&seqGo=${encodeURIComponent(valeur('seqGo'))}&seqBack=${encodeURIComponent(valeur('seqBack'))}&uMark=${valeur('uMark')}&uInter=${valeur('uInter')}&parkB=${valeur('parkB')}&parkC=${valeur('parkC')}&finalMark=${valeur('finalMark')}&finalInter=${valeur('finalInter')}`);
      }, 120);
    }
...
bool executerPauseMarqueur()
{
  if (!pauseMarqueurActive) {
    return false;
  }

  arreterMoteurs();
  if (millis() - debutPauseMarqueurMs >= dureePauseMarqueurMs) {
    pauseMarqueurActive = false;
  }
...
void detecterMarqueurs()
{
  ...
  if ((modeArretMarqueur || modePauseMarqueur || modeParcoursAvance) && robotActif &&
    ...
    if (modePauseMarqueur) {
      pauseMarqueurActive = true;
      debutPauseMarqueurMs = maintenant;
    }
  }
...
void chargerReglages()
{
  ...
  modePauseMarqueur = preferences.getBool("pauseM", modePauseMarqueur);
  dureePauseMarqueurMs = preferences.getInt("pauseMs", dureePauseMarqueurMs);
...
void sauvegarderReglages()
{
  ...
  preferences.putBool("pauseM", modePauseMarqueur);
  preferences.putInt("pauseMs", dureePauseMarqueurMs);
...
void envoyerEtat()
{
  ...
  json += ",\"pauseMarkMode\":";
  json += modePauseMarqueur ? "true" : "false";
  json += ",\"pauseMarkActive\":";
  json += pauseMarqueurActive ? "true" : "false";
  json += ",\"pauseMarkMs\":";
  json += dureePauseMarqueurMs;
  json += ",\"count\":";
  ...
  if (demiTourActif) {
    json += "Demi-tour";
  } else if (pauseParkingActive || pauseMarqueurActive) {
    json += "Pause";
...
void appliquerReglagesDepuisWeb()
{
  ...
  if (serveur.hasArg("pauseMark")) {
    modePauseMarqueur = serveur.arg("pauseMark").toInt() == 1;
  }
  if (serveur.hasArg("pauseMarkMs")) {
    dureePauseMarqueurMs = constrain(serveur.arg("pauseMarkMs").toInt(), 500, 5000);
  }
...
void configurerInterfaceWeb()
{
  ...
  serveur.on("/toggle", []() {
    ...
    if (!etaitActif && robotActif && (modeArretMarqueur || modePauseMarqueur)) {
      compteurMarqueurs = 0;
      marqueurDejaCompte = false;
      pauseMarqueurActive = false;
    }
