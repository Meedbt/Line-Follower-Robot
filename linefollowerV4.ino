  ...
  <script>
    ...
    function majReglages() {
      ...
      envoiTimer = setTimeout(() => {
        fetch(`/set?base=${valeur('base')}&kp=${valeur('kp')}&kd=${valeur('kd')}&mode=${valeur('mode')}&target=${valeur('target')}&us=${valeur('us')}&usDist=${valeur('usDist')}&inter=${valeur('inter')}&seq=${encodeURIComponent(valeur('seq'))}`);
      }, 120);
    }
...
char decisionPourIntersection(int numeroIntersection)
{
  ...
  for (uint16_t i = 0; i < sequenceIntersections.length(); i++) {
    char c = sequenceIntersections.charAt(i);
    if (c == 'S' || c == 'G' || c == 'D') {
      derniereDecision = c;
...
void detecterIntersections()
{
  ...
  if (!modeIntersections || actionIntersectionActive) {
    return;
  }
...
void detecterMarqueurs()
{
  ...
  if (modeArretMarqueur && robotActif &&
...
void loop()
{
  ...
  if (modeArretMarqueur && compteurMarqueurs >= cibleMarqueurs) {
    robotActif = false;
    arreterMoteurs();
    return;
  }
