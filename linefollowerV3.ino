  ...
  <script>
    ...
    function majReglages() {
      ...
      envoiTimer = setTimeout(() => {
        fetch(`/set?base=${valeur('base')}&kp=${valeur('kp')}&kd=${valeur('kd')}&mode=${valeur('mode')}&target=${valeur('target')}&us=${valeur('us')}&usDist=${valeur('usDist')}`);
      }, 120);
    }
...
void detecterMarqueurs()
{
  ...
  for (uint8_t i = 0; i < NOMBRE_CAPTEURS; i++) {
    if (valeursCapteurs[i] > SEUIL_MARQUEUR) {
      nbCapteursNoirs++;
    }
  }
